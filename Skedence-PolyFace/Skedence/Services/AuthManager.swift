...
final class AuthManager: ObservableObject {
    // 🏷️ WHITE-LABEL: Hard-coded organization ID for PolyFace Volleyball Academy
    private static let HARDCODED_ORG_ID = "polyface_volleyball_academy_"
    ...
    // 🏷️ WHITE-LABEL: Hard-coded PolyFace brand color
    @Published var primaryColor: Color = Color(red: 0.196, green: 0.345, blue: 0.639) // PolyFace Blue #3258A3
    ...
    func register(email: String,
                  ...
                  phoneNumber: String?,
                  orgId: String? = nil) async -> Bool { // 🏷️ WHITE-LABEL: orgId parameter kept for compatibility but ignored
        authError = nil // Clear any stale errors before starting
        do {
            ...
            // 🏷️ WHITE-LABEL: Use hard-coded org ID
            let polyfaceOrgId = AuthManager.HARDCODED_ORG_ID
            ...
            let data: [String: Any?] = [
                ...
                "photoURL": nil,
                "orgId": polyfaceOrgId, // 🏷️ WHITE-LABEL: Use hard-coded org ID
                "active": true,
                "createdAt": now,
                "updatedAt": now
            ]
            ...
            // 🏷️ WHITE-LABEL: Always create orgMembers entry with hard-coded org ID
            ...
            let memberDocId = "\(authUid)_\(polyfaceOrgId)"
    ...
    func signOut() {
        do {
            ...
            // 🏷️ WHITE-LABEL: Reset to PolyFace brand color
            primaryColor = Color(red: 0.196, green: 0.345, blue: 0.639) // PolyFace Blue #3258A3
    ...
    private func loadUserData(for authUserId: String) async {
        // 🏷️ WHITE-LABEL: Use hard-coded org ID
        currentOrgId = AuthManager.HARDCODED_ORG_ID
        ...
        do {
            ...
            if let doc = snapshot.documents.first {
                ...
                await loadOrgBranding(orgId: AuthManager.HARDCODED_ORG_ID)
                return
            }
            ...
            if let userDoc = userSnapshot.documents.first {
                // let data = userDoc.data() // <--- Removed this line (unused variable)
    ...
    private func loadOrgBranding(orgId: String) async {
        do {
            ...
            if let branding = orgData["branding"] as? [String: Any] {
                // 🏷️ WHITE-LABEL: Primary color is hard-coded, not loaded from Firestore
                // if let colorHex = branding["primaryColor"] as? String {
                //     primaryColor = Color(hex: colorHex) ?? Color(red: 0.196, green: 0.345, blue: 0.639)
                // }
