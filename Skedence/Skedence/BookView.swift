    ...
    var body: some View {
        NavigationView {
            ScrollView {
                VStack(alignment: .leading, spacing: Spacing.xl) {
                    ...
                    // Athlete Selection UI (only show if not yet registered successfully)
                    if !registrationSuccessful && !classItem.isFull && !availableClassPasses.isEmpty {
                        ...
                        // Is She the Only One? (only show if athlete is selected)
                        if selectedAthleteName != nil {
                            VStack(alignment: .leading, spacing: Spacing.md) {
                                ...
                                HStack(spacing: Spacing.sm) {
                                    Button {
                                        ...
                                        .background(
                                            RoundedRectangle(cornerRadius: CornerRadius.md)
                                                .stroke(isOnlyParticipant == true ? AppTheme.success : AppTheme.border, lineWidth: 2)
                                                .background(
                                                    RoundedRectangle(cornerRadius: CornerRadius.md)
                                                        .fill(isOnlyParticipant == true ? AppTheme.success.opacity(0.1) : Color.platformSecondaryBackground)
                                                )
                                        )
                                    }
                                    .buttonStyle(.plain)
                                    ...
                                    Button {
                                        ...
                                        .background(
                                            RoundedRectangle(cornerRadius: CornerRadius.md)
                                                .stroke(isOnlyParticipant == false ? AppTheme.success : AppTheme.border, lineWidth: 2)
                                                .background(
                                                    RoundedRectangle(cornerRadius: CornerRadius.md)
                                                        .fill(isOnlyParticipant == false ? AppTheme.success.opacity(0.1) : Color.platformSecondaryBackground)
                                                )
                                        )
                                    }
                                    .buttonStyle(.plain)
                        ...
                        // Second Athlete Selection (only show if multiple participants)
                        if isOnlyParticipant == false {
                            VStack(alignment: .leading, spacing: Spacing.md) {
                                Text("Select second participant")
                                    .font(.headingMedium)
                                ...
                                CardView(padding: Spacing.md) {
                                    Menu {
                                        ...
                                        Button {
                                            secondAthleteName = "New Athlete"
                                            isNewAthlete = true
                                            newAthleteIntakeData.resetFields()
                                        } label: {
                                            HStack {
        ...
        .task {
            ...
            if let orgId = auth.currentOrgId {
                await pricingService.loadPricingStructure(for: orgId)
                await settingsService.loadSettings(orgId: orgId)
                await intakeFormService.loadIntakeFormConfig(orgId: orgId)
            }
        }
        .sheet(isPresented: $showWaiverAgreement) {
            WaiverAgreementCheckboxView(
