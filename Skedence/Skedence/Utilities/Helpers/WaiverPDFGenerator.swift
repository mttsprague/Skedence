...
struct WaiverPDFGenerator {
    ...
    static func generateWaiverPDF(signature: WaiverSignature, organizationName: String = "Your Organization", customWaiverText: String? = nil, athleteName: String? = nil) -> Data? {
        ...
        let data = renderer.pdfData { context in
            ...
            // Footer Section with Summary - Make this more prominent
            ...
            let summaryHeaderAttributes: [NSAttributedString.Key: Any] = [
                .font: UIFont.boldSystemFont(ofSize: 12),
                .foregroundColor: UIColor.black
            ]
            ...
            // Summary title
            "WAIVER SUMMARY".draw(at: CGPoint(x: leftMargin, y: currentY), withAttributes: summaryHeaderAttributes)
            currentY += 20
            
            // Athlete Name
            if let athleteName = athleteName {
                "Athlete Name: ".draw(at: CGPoint(x: leftMargin, y: currentY), withAttributes: summaryHeaderAttributes)
                athleteName.draw(at: CGPoint(x: leftMargin + 110, y: currentY), withAttributes: summaryValueAttributes)
                currentY += 18
            }
            
            // Guardian/Participant Name
            let guardianLabel = signature.isMinor ? "Parent/Guardian: " : "Participant: "
            guardianLabel.draw(at: CGPoint(x: leftMargin, y: currentY), withAttributes: summaryHeaderAttributes)
            signature.fullName.draw(at: CGPoint(x: leftMargin + 110, y: currentY), withAttributes: summaryValueAttributes)
            currentY += 18
            
            // Signature
            "Signature: ".draw(at: CGPoint(x: leftMargin, y: currentY), withAttributes: summaryHeaderAttributes)
            let italicSigAttributes: [NSAttributedString.Key: Any] = [
                .font: UIFont.italicSystemFont(ofSize: 11),
                .foregroundColor: UIColor.blue
            ]
            signature.fullName.draw(at: CGPoint(x: leftMargin + 110, y: currentY), withAttributes: italicSigAttributes)
            currentY += 18
            
            // Date
            "Date Agreed: ".draw(at: CGPoint(x: leftMargin, y: currentY), withAttributes: summaryHeaderAttributes)
            let shortDateFormatter = DateFormatter()
            shortDateFormatter.dateStyle = .medium
            let shortDateString = shortDateFormatter.string(from: signature.signedAt)
            shortDateString.draw(at: CGPoint(x: leftMargin + 110, y: currentY), withAttributes: summaryValueAttributes)
            ...
            let footerRect = CGRect(x: leftMargin, y: pageHeight - 50, width: contentWidth, height: 40)
