               ...
               let orgId = auth.currentOrgId {
                Task {
                    try? await ActivityLogger.shared.log(
                        ...
                        targetType: "pass",
                        description: "\(user.displayName ?? "Client") purchased \(selectedPackage.title) (\(selectedPackage.lessonCount) sessions) for $\(String(format: "%.2f", Double(selectedPackage.priceInCents) / 100.0))",
                        metadata: [
                            "passId": result.paymentIntentId,
                            "passType": selectedPackage.packageType,
                            "sessionsCount": selectedPackage.lessonCount,
