...
## Overview
Complete Stripe payment integration has been implemented across Skedence with multi-athlete lessons, class registration payments, and comprehensive profile tracking.
...
**Files Modified:**
- `Skedence/PurchaseLessonsView.swift` - Added `twoAthlete` and `threeAthlete` cases to PackageOption enum
- `Skedence Admin/functions/src/stripe.ts` - Added validation for two_athlete (14000¢) and three_athlete (18000¢)
...
**Files Modified:**
- `Skedence/BookView.swift` - Added StripeService, PaymentSheet state, payment flow
...
**Files Modified:**
- `Skedence/ProfileView.swift` - Replaced single passes count with `passTypeCard()` function
...
**Files Modified:**
- `Skedence/ProfileView.swift` - Added ClassesService integration
