...
class AddTrainerViewModel: ObservableObject {
    ...
    func addTrainer(
        ...
        do {
            ...
            // Create Firebase Auth user (optional - they can sign up themselves)
            // For now, we'll just create the trainer document
            
            // If they already have a user account, create orgMembers entry
            ...
            if let userDoc = userQuery.documents.first {
                ...
                try await db.collection("orgMembers")
                    .document("\(userId)_\(orgId)")
                    .setData(memberData)
                print("✅ Created orgMember for existing user: \(userId)")
            }
